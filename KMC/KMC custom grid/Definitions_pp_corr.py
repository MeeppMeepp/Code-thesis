...
def create_arrays(grid_points):
    ...
    for index, (x, y) in enumerate(grid_points):
        ...
        col_index = x / a_grid  # Determine the column number
        if ((index // 8) % 2) % 2 == 0:  # Even row: Start with pink
            if (row_index) % 2 == 0:
                if index % 2 == 0:
...
def create_rates(grid_points, neighbor, block_height, alpha, beta, k_des, k_ads, k_diff, k_diff_hor, k_diff_ab,
    ...
    for x in range(len(grid_points)):
        ...
        vertical,short, long = lateral_neighbors[x].values()
        ...
        if (block_height[short] == block_height[x]) or (block_height[short] + 1 == block_height[x]):
            k_diff_ab[x] = ftrial_diff * (xs / a) ** 2 * (numpy.exp(alpha / 4 * (2 + neighbor[short] - i) - beta / 2))

        if (block_height[long] == block_height[x]) or (block_height[long] + 1 == block_height[x]):
            k_diff_below[x] = ftrial_diff * (xs / a) ** 2 * (
                numpy.exp(alpha / 4 * (2 + neighbor[long] - i) - beta / 2))
...
def diffusion_nb(rnd, xpos, k_diff_hor, k_diff_ab, k_diff_below, block_height, neighbor, lateral_neighbors):
    vertical, short, long = lateral_neighbors[xpos].values()
    ...
    if rnd <= k_diff_hor[xpos]:  # horizontal
        ...
        if (block_height[short] == block_height[xpos]) or (block_height[short] + 1 == block_height[xpos]):
            neighbor[xpos] = 0
            block_height[short] += 1
            block_height[xpos] -= 1
            neighbor[short] = 0
            migrated_position = [xpos, short]
            block_height, neighbor = neighbor_change_diffusion(lateral_neighbors, migrated_position, block_height, xpos,
                                                              neighbor)
        else:
            migrated_position = [xpos]
        ...
        if (block_height[long] == block_height[xpos]) or (block_height[long] + 1 == block_height[xpos]):
            neighbor[xpos] = 0
            block_height[long] += 1
            block_height[xpos] -= 1
            neighbor[long] = 0
            migrated_position = [xpos, long]
            block_height, neighbor = neighbor_change_diffusion(lateral_neighbors, migrated_position, block_height, xpos,
                                                              neighbor)
        else:
            migrated_position = [xpos]
    ...
    for x in range(len(grid_points)):
        ...
        if (block_height[short] == block_height[x]) or (block_height[short] + 1 == block_height[x]):
            k_diff_ab[x] = ftrial_diff * (xs / a) ** 2 * (
                numpy.exp(alpha / 4 * (2 + neighbor[short] - i) - beta / 2))
